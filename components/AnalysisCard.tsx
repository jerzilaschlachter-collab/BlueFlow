...
const BIAS_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  bullish: { bg: 'bg-green-500/15', text: 'text-green-600', label: 'BULLISH' },
  bearish: { bg: 'bg-red-500/15', text: 'text-red-500', label: 'BEARISH' },
  neutral: { bg: 'bg-yellow-500/15', text: 'text-yellow-600', label: 'NEUTRAL' },
}
    ...
    <div className="bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden shadow-[0_1px_4px_rgba(0,0,0,0.06)]">
      ...
      <div className="p-6 grid md:grid-cols-2 gap-6">
        ...
        <div className="space-y-5">
          {/* Trend */}
          <div>
            <p className="text-[#94A3B8] text-xs uppercase tracking-widest mb-2">Trend</p>
            <p className="text-[#0A0E27] text-sm leading-relaxed">{analysis.trend}</p>
          </div>

          {/* Pattern */}
          <div>
            <p className="text-[#94A3B8] text-xs uppercase tracking-widest mb-2">Pattern</p>
            <p className="text-[#0A0E27] text-sm leading-relaxed">{analysis.pattern}</p>
          </div>

          {/* Confidence Score */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-[#94A3B8] text-xs uppercase tracking-widest">Confidence Score</p>
              <span className="text-[#0A0E27] font-bold text-lg tabular-nums">
                {analysis.confidence_score}
                <span className="text-[#64748B] text-sm font-normal">%</span>
              </span>
            </div>
          ...
          {/* Key Levels */}
          <div>
            <p className="text-[#94A3B8] text-xs uppercase tracking-widest mb-3">Key Levels</p>
            <div className="grid grid-cols-2 gap-3">
              {/* Support */}
              <div className="bg-green-500/5 border border-green-500/15 rounded-xl p-3">
                <p className="text-green-600 text-xs font-semibold uppercase mb-2">Support</p>
                <ul className="space-y-1">
                  {(analysis.key_levels?.support ?? []).map((level, i) => (
                    <li key={i} className="text-[#0A0E27] text-xs">
                      {level}
                    </li>
                  ))}
                </ul>
        ...
        <div>
                ...
                >
                  ...
                  <span
                    className={`text-sm leading-relaxed ${
                      checked ? 'text-[#94A3B8] line-through' : 'text-[#0A0E27]'
                    }`}
                  >
                    {item}
                  </span>
          ...
          {checkedItems.size === (analysis.pre_trade_checklist?.length ?? 0) &&
            checkedItems.size > 0 && (
              <div className="mt-4 flex items-center gap-2 bg-green-500/10 border border-green-500/20 rounded-xl px-4 py-3">
                ...
                <p className="text-green-600 text-sm font-medium">All conditions met — ready to trade</p>
              </div>
            )}
          ...
          {analysis.image_url && (
            <div className="mt-5">
              <p className="text-[#94A3B8] text-xs uppercase tracking-widest mb-2">Chart</p>
              <img
                src={analysis.image_url}
                alt="Analyzed chart"
                className="w-full rounded-xl object-cover max-h-40 bg-[#F1F5F9]"
              />
            </div>
          )}
        </div>
